...
const CardToCardReciept = ({ownerName , transactionDate, initialCard, desCard, amount, transactionStatus, onBack }) => {
  ...
  >
    <Container sx={{ height: {xs:'auto' , sm:'100vh'},display: 'flex', justifyContent: 'center', alignItems: 'center', paddingBottom:{xs:20,sm:46}, paddingTop:{xs:10, sm:38} }}>

    <Box sx={{ maxWidth: 500,width:{xs:500 , sm:400} ,mx: 'auto', paddingY: 0, paddingX: 0, borderRadius: 2, boxShadow: 3, bgcolor: 'white' }}>
      <Box sx={{ textAlign: 'center', mb: 1 }}>
        <Typography variant="h5" sx={{ fontWeight: 'bold', color: transactionStatus === true ? 'green' : 'red' , border: `1.4px solid ${transactionStatus === true ? '#b6e9d2' : 'red'}`,borderRadius:'4px' , p:2}}>
      ...
      <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between', borderBottom: '1px dashed gray', paddingY: 1, color: '#56575b'  }}>
        ...
        <Typography sx={{fontSize:'1.1rem'}}>انتقال وجه کارت به کارت</Typography>
      </Box>
      <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between', borderBottom: '1px dashed gray', paddingY: 1, color: '#56575b'  }}>
      ...
      <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between', borderBottom: '1px dashed gray', paddingY: 1, color: '#56575b' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Typography sx={{fontSize:'1.1rem'}}>{toPersianNumbers(maskCardNumber(formatCardNumber(initialCard)))}</Typography>
      ...
      <Box sx={{ mb: 1, display: 'flex', justifyContent: 'space-between', borderBottom: '1px dashed gray', paddingY: 1, color: '#56575b' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center' }}>
          ...
          <Typography sx={{fontSize:'1.1rem'}}>{toPersianNumbers(maskCardNumber(formatCardNumber(desCard)))}</Typography>
          {desBankInfo?.icon && (
            <Box sx={{ marginLeft: '4px' }}>
      ...
      <Box sx={{ display: 'flex', justifyContent: 'center',alignItems:'center', paddingY: 1, color: transactionStatus === true ? 'green' :'#56575b'  }}>
        <Typography sx={{fontSize:'1.2rem'}}>اشتراک گذاری</Typography>
        <ShareIcon onClick={handleShare} style={{ marginRight: '10px', cursor: 'pointer' , fontSize:'1.2rem' }} />
      </Box> 
